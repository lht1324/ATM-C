...
int create_account(int count) // 계좌를 새로 생성하는 함수이다.
{	// count = 총 고객의 수이다.
	int i, account_check = 0, end;
	// i : 반복문에 사용되는 변수이다. 
	// account_check : 계좌번호 생성 시 기존 계좌번호 중
	// 같은 번호가 있는지 없는지 확인할 때 사용되는 변수이다.
	// end : 계좌 생성 후 초기 화면으로 돌아갈 지 여부를 물을 때 사용되는 변수이다.
	...
	// 이름, 계좌번호, 잔고가 파일에 쓰여질 때 앞의 5자리엔 비밀번호 4자리와 공백 1자리가
	// 쓰인다. 3개의 파일에 쓰인 비밀번호가 모두 맞다면 파일은 변조되지 않은 것이다.
	FILE *afp1;
	FILE *afp2;
	FILE *afp3;
	// 파일 포인터 afp1, afp2, afp3를 선언한다.
	fopen_s(&afp1, "acc.txt", "a");
	fopen_s(&afp2, "name.txt", "a");
	fopen_s(&afp3, "balance.txt", "a");
	// acc.txt, name.txt, balance.txt를 추가 모드로 연다.
	printf("성함을 입력해주세요 : ");
	scanf_s("%s", name, 13); // 이름을 입력받는다.

	name[strlen(name)] = '\0'; // name의 마지막 원소에 NULL 값을 대입한다.
	...
	// 시간에 따라 다른 숫자를 출력하게 해준다. 이게 없다면 랜덤값이 한 번만 출력된다.
	while (account_check == 0)
	{
		...
		str_a[21] = '\0';
		fp[16] = '\0';
		// str_a, fp의 마지막 원소에 NULL 값을 대입한다.
		for (i = 0; i < count; i++)
		{
	...
	str_b[6] = '\0'; // str_b의 마지막 원소에 NULL 값을 대입한다.
	...
	while (1)
	{
		...
		scanf_s("%d", &end);
		if (end == 0) // end에 0을 입력받으면 create_account() 함수를 종료한다.
			return 0;
		else
			printf("\n잘못 입력하셨습니다.\n\n");
	}
