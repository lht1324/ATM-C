...
int withdraw(int client) // 출금을 담당하는 함수이다.
{
	int i, len, result, end;
	// i : 반복문에 사용되는 변수이다. len : 고객의 잔고의 길이를 저장할 변수이다.
	// result : 출금한 결과를 저장할 변수이다.
	// end : 출금 후 고객에게 초기 화면으로 갈 지 물어볼 때 사용되는 변수이다.
	char c_money[16], w_money[16], b_money[16], bal[21];
	// c_money : client_money(기존 잔고)
	// w_money : withdraw_money(출금할 돈)
	...
	scanf_s("%s", w_money, 16); // 출금할 금액을 입력받는다.

	if (result < atoi(w_money))
	{
		printf("\n잔고가 부족합니다.\n");
		return 0;
	}
		...
		result -= atoi(w_money);
	// 출금할 금액인 w_money를 정수로 바꿔 result에서 뺀다.
